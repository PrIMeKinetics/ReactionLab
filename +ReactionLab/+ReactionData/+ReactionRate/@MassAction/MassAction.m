classdef MassAction < ReactionLab.ReactionData.ReactionRate.RateCoefficient ...
                    & ReactionLab.ReactionData.ReactionRate.Arrhenius
...
% Last modified: January 1, 2011
   ...
   properties
      Order = [];   % reaction order,  MassActionRK.Order = Reaction.Order
   end

   methods
      function obj = MassAction(varargin)
         if nargin > 0
            if isa(varargin{1},'System.Xml.XmlDocument')
               rk = ReactionLab.ReactionData.ReactionRate.MassAction();
               rk.Order = varargin{2};  %  rk.Order = rxn.Order
