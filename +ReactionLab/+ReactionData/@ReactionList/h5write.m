function h5write(rxnList,h5path)
% h5write(ReactionListObj,HDF5path)
%                                                               
% create reaction part of the HDF5 file

% Copyright 1999-2013 Michael Frenklach
% Created:   November 20, 2010, myf
% Last modified: April 4, 2013, myf: changed hdf5 to h5

rxn = rxnList.Values;

% indentifying species in reactions
speNames    = h5read(h5path,'/speciesData/speciesNames');
spePrimeIds = h5read(h5path,'/speciesData/speciesPrimeID');
...
h5write(h5path,'/reactionData/reactionSpeciesIndex',sort(rxnSpeInd),...            
...
%    reactant = get(rxn(iRxn),'species');
   ...
   try
       h5write(h5path,['/reactionData/rateCoefLinks/' rxn(iRxn).PrimeId],rkLink,...
                       'WriteMode', 'append'                                     );
   end
   waitbar(iRxn/nRxn);
end
...
h5write(h5path,'/reactionData/reactantSpeciesIndex',forwStoich',...
...
h5write(h5path,'/reactionData/reactionRateType',  rxnType,           ...
